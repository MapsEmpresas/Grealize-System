...
function ListaClienteSite(props) {
  ...
  const sortedClientes = React.useMemo(() => {
    return props.arrayClientesSite.slice().sort((a, b) => new Date(b.data) - new Date(a.data));
  }, [props.arrayClientesSite]);
  ...
  const handleVerificarPagos = useCallback(async () => {
    try {
      ...
      if (userId === 'WcOsPxuR4fMICQTnu2m7r0Abdf23' || userId === 'rrMhvTLAElMAI0l7j0T2y9Ypm842') {
        setIsAdmUser(true)
      }
      else if (userId === 'WcOsPxuR4fMICQTnu2m7r0Abdf23'|| userId === 'rrMhvTLAElMAI0l7j0T2y9Ypm842') {
        setExclusiveUser(true)
      }
      let q;
      const userAllViwer = ((userId === 'rrMhvTLAElMAI0l7j0T2y9Ypm842') || (userId === 'WcOsPxuR4fMICQTnu2m7r0Abdf23') || (userId === 'ghJv0yz2lVgxIjvVJv9wMO6Fpmh2'))
      if (userAllViwer) {
        q = query(collection(db, 'clientessite'));
      } else {
        q = query(collection(db, 'clientessite'), where('userId', '==', userId));
      }
        ...
        <tbody>
          {formattedClientes.filter((cliente) => (!filtroDataVenda01 || cliente.data === filtroDataVenda01) && (!filtroDataVenda02 || cliente.venc2 === filtroDataVenda02)).map((cliente) => (
            <tr key={cliente.id} className="table-light">
              <th scope="row" className="align-middle"><Link to={`/app/home/fichaclientesite/${cliente.id}`} className="fa-solid fa-list icone-acao1 align-middle"></Link>{cliente.cpf}</th>
              ...
              <td>
                <Link to={`/app/home/editarclientesite/${cliente.id}`}><i className="fa-solid fa-pen-to-square icone-acao"></i></Link>
...
export default ListaClienteSite;